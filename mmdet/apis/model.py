...
class ModelHub:
    ...
    @classmethod
    def _register_mmdet_models(cls):
        # register models in mmdetection
        if not cls.__mmdet_registered:
            from importlib_metadata import distribution
            root = distribution('mmdet').locate_file('mmdet')
            model_index_path = root / '.mim' / 'model-index.yml'
            ModelHub.register_model_index(
                model_index_path, config_prefix=root / '.mim')
            cls.__mmdet_registered = True
...
def list_models(pattern=None, exclude_patterns=None) -> List[str]:
    """List all models available in mmdetection.
    ...
    """
    ModelHub._register_mmdet_models()
